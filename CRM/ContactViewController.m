...
-(void)doWhenEcardGetInfoFromWebServier:(NSString *)soapresult
{
    ...
    NSError *error=nil;
    //解析数据
    self.contactOfDic=[NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:&error];
    //把解析的数据给数组
    self.dataOfContact=[self.contactOfDic objectForKey:@"联系人表"];
#warning 检查数据
   // NSLog(@"%@",soapresult);
}
